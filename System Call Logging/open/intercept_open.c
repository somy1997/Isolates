...
// do for close, read, write

static asmlinkage long my_open(const char __user *filename, int flags, umode_t mode)
{
    asmlinkage long (*tmp_open)(const char __user *filename, int flags, umode_t mode);
    printk(KERN_WARNING "ISOLATES:open,%s,%d,%d\n", current->comm, current->pid, current->cred->uid.val);
    //pr_WARNING("%s\n",__func__);
    tmp_open = (asmlinkage long (*)(const char __user *filename, int flags, umode_t mode)) old_sys_table[__NR_open];
    return tmp_open(filename, flags, mode);
}
